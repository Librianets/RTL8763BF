...
/**
 * @brief  HID Host Link control block defination.
 */
typedef struct
{
...
bool hids_read_saved_data(uint8_t conn_id, T_HIDS_READ_TYPE read_type, T_HIDS_READ_VALUE *p_value)
{
    ...
    case HIDS_READ_REPORT_MAP:
        {
            if (hids_client_table[conn_id].read_flags & HIDS_READ_REPORT_MAP)
            {
            ...
            else if (handle == hdl_cache[HDL_HIDS_CHAR_REPORT_MAP])
            {
                if (value_size != 0)
                {
                    ...
                    if (hids_client_table[conn_id].p_report_map != NULL)
                    {
                        ...
                        PROFILE_PRINT_INFO0("[HIDS]: Read report map success");
                    }
                    else
                    {
                        cause = GATT_ERR | GATT_ERR_INVALID_PARAM;
                        PROFILE_PRINT_ERROR0("[HIDS]: Read report map failed, allocate memory failed");
                    }
                }
                else
                {

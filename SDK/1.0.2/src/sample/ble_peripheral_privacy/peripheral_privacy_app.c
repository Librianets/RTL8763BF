...
void app_adv_start(void)
{
    ...
    if (p_entry == NULL)
    {
        /* No bonded device, send connectable undirected advertisement*/
        app_work_mode = APP_PAIRABLE_MODE;
        if (app_privacy_resolution_state == LE_PRIVACY_RESOLUTION_ENABLED)
        {
            le_privacy_set_addr_resolution(false);
        }
    ...
    else
    {
        ...
        if (((p_entry->remote_bd.remote_bd_type == GAP_REMOTE_ADDR_LE_PUBLIC) ||
            ...
            && (high_duty_adv_count < HIGH_DUTY_ADV_MAX_COUNT)
            && (le_check_privacy_bond(p_entry) == false)
           )
        {
            high_duty_adv_count++;
        ...
        else
        {
            ...
            if (app_privacy_resolution_state == LE_PRIVACY_RESOLUTION_DISABLED)
            {
    ...
    if (adv_evt_type == GAP_ADTYPE_ADV_IND)
    {
        high_duty_adv_count = 0;
    }

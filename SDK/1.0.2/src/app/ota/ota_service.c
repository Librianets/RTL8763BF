    ...
    case BLE_SERVICE_CHAR_DEVICE_INFO_INDEX:
        {
            ...
            DeviceInfo.secure_version = p_header->ctrl_header.secure_version;
            DeviceInfo.mode.buffercheck = mode & 0x1;
    ...
    case BLE_SERVICE_CHAR_IMAGE_VERSION_INDEX:
        {
            ...
            uint32_t addr;
            uint8_t number = 0, length = 8;
            if (is_ota_support_bank_switch())
            {
                T_OTA_HEADER_FORMAT *p_otaheader;
                addr = get_header_addr_by_img_id(OTA);
                PROFILE_PRINT_INFO1("otaheader_addr = %x", addr);
                p_otaheader = (T_OTA_HEADER_FORMAT *)addr;
                version[number] = p_otaheader->ver_val;
                PROFILE_PRINT_INFO1("otaheader_version = %x", version[number]);
                number++;
                length += 4;
            }
            ...
            version[number] = p_imgheader->git_ver.ver_info.version;
            PROFILE_PRINT_INFO1("patch_version = %x", version[number]);
            number++;
            ...
            p_imgheader = (T_IMG_HEADER_FORMAT *)addr;
            version[number] = p_imgheader->git_ver.ver_info.version;
            PROFILE_PRINT_INFO1("app_version = %x", version[number]);
            number++;

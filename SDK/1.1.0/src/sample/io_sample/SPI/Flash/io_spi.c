...
{
    uint8_t cmd = 0x9F;
    APP_PRINT_INFO0("[io_spi] spi_demo: Get fm25q16a chip id. ");
    /* Get flash id. */
    fm25q16a_read_data(&cmd, 1);
}
...
void io_spi_handle_msg(T_IO_MSG *io_spi_msg)
{
    ...
    APP_PRINT_INFO1("[io_spi]io_spi_handle_msg: data_lenth = %d ", data_lenth);
    for (uint8_t i = 0; i < data_lenth; i++)
    {

...
#include "os_sched.h"
/*
 *格式：#命令#!conn_id![bd_addr]<执行结果>\r\n
 *参数：
 *	   命令："scan","stopscan","showdev"
 *	   conn_id：0~6
 *	   bd_addr：[685a947e1abe]
 *	   执行结果：Y=succeed,N=fail
 *例子：
 *	   #showdev#!0![685a947e1abe]<Y>\r\n
*/
...
char cmd_buffer[FRAMING_PAYLOAD_MAX_LENGTH]={0};
...
static T_USER_CMD_PARSE_RESULT cmd_showcon(T_USER_CMD_PARSED_VALUE *p_parse_value)
{
    ...
    for (conn_id = 0; conn_id < APP_MAX_LINKS; conn_id++)
    {
        ...
        {
#if 0   //by vsop     	
      data_uart_print("#showcon#!%d![%02x%02x%02x%02x%02x%02x]<Y>\r\n",conn_id,
...
#else
	sprintf(cmd_buffer,"#showcon#!%d![%02x%02x%02x%02x%02x%02x]<Y>\r\n",conn_id,
		conn_info.remote_bd[5], conn_info.remote_bd[4],
		conn_info.remote_bd[3], conn_info.remote_bd[2],
		conn_info.remote_bd[1], conn_info.remote_bd[0]);
...
/**
 * @brief Clear bonded devices information by connect id
 *
 * <b>Command table define</b>
 * \code{.c}
    {
        "bonddel",
        "bonddel [conn_id]\n\r",
        "Clear bonded devices information by connect id\n\r",
        cmd_bonddel
    },
 * \endcode
 */
static T_USER_CMD_PARSE_RESULT cmd_bonddel(T_USER_CMD_PARSED_VALUE *p_parse_value)
{
    uint8_t conn_id = p_parse_value->dw_param[0];
    T_GAP_CAUSE cause;
    cause = le_bond_delete_by_idx(conn_id);
    return (T_USER_CMD_PARSE_RESULT)cause;
}

/**
 * @brief use for pcba autotest ble module uart test
 *
 * <b>Command table define</b>
 * \code{.c}
    {
        "uarttest",
        "uarttest\n\r",
        "for autotest uart port test\n\r",
        uarttest

    },
 * \endcode
 */
static T_USER_CMD_PARSE_RESULT uarttest(T_USER_CMD_PARSED_VALUE *p_parse_value)
{
	data_uart_print("bt uart test ok\r\n");
     return (RESULT_SUCESS);
}
...
static T_USER_CMD_PARSE_RESULT cmd_bondinfo(T_USER_CMD_PARSED_VALUE *p_parse_value)
{
    ...
    for (i = 0; i < bond_storage_num; i++)
    {
        ...
        {
#if 0        
          	data_uart_print("#bondinfo#!%d![%02x%02x%02x%02x%02x%02x]<Y>\r\n",
							p_entry->idx,
...
#else
			sprintf(cmd_buffer,"#bondinfo#!%d![%02x%02x%02x%02x%02x%02x]<Y>\r\n",
							p_entry->idx,
							p_entry->remote_bd.addr[5],
							p_entry->remote_bd.addr[4],
							p_entry->remote_bd.addr[3],
...
static T_USER_CMD_PARSE_RESULT cmd_showdev(T_USER_CMD_PARSED_VALUE *p_parse_value)
{
    ...
    {
#if 0 //by vsop    
       data_uart_print("#showdev#!%d![%02x%02x%02x%02x%02x%02x]<Y>\r\n",
						i,
...
#else
		sprintf(cmd_buffer,"#showdev#!%d![%02x%02x%02x%02x%02x%02x]<Y>\r\n",
						i,
						dev_list[i].bd_addr[5], dev_list[i].bd_addr[4],
						dev_list[i].bd_addr[3], dev_list[i].bd_addr[2],
						dev_list[i].bd_addr[1], dev_list[i].bd_addr[0]);
	...
	{
		#if 0
		data_uart_print("#showdev#<Y>\r\n");
		#else
		strcpy(cmd_buffer,"#showdev#<Y>\r\n");
		FramingWrapFrameUartPrint((uint8_t *)cmd_buffer, strlen(cmd_buffer));
...
static T_USER_CMD_PARSE_RESULT cmd_simpwritev2(T_USER_CMD_PARSED_VALUE *p_parse_value)
{
    ...
    for (i = 0; i < length; i++)
    {
        data[i] = p_parse_value->dw_param[3+i];
    }

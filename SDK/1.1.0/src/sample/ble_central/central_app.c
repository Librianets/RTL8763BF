...
#if 0 //by vsop	
           data_uart_print("#disc#!%d![%02x%02x%02x%02x%02x%02x]<Y>\r\n",
						conn_id,
						app_link_table[conn_id].bd_addr[5], app_link_table[conn_id].bd_addr[4],
						app_link_table[conn_id].bd_addr[3], app_link_table[conn_id].bd_addr[2],
						app_link_table[conn_id].bd_addr[1], app_link_table[conn_id].bd_addr[0]);
#else
			sprintf(cmd_buffer,"#disc#!%d![%02x%02x%02x%02x%02x%02x]<Y>\r\n",
						conn_id,
						app_link_table[conn_id].bd_addr[5], app_link_table[conn_id].bd_addr[4],
						app_link_table[conn_id].bd_addr[3], app_link_table[conn_id].bd_addr[2],
						app_link_table[conn_id].bd_addr[1], app_link_table[conn_id].bd_addr[0]);
				FramingWrapFrameUartPrint((uint8_t *)cmd_buffer, strlen(cmd_buffer));
...
#if 0 //by vsop			
           data_uart_print("#condev#!%d![%02x%02x%02x%02x%02x%02x]<Y>\r\n",
						conn_id,
						app_link_table[conn_id].bd_addr[5], app_link_table[conn_id].bd_addr[4],
						app_link_table[conn_id].bd_addr[3], app_link_table[conn_id].bd_addr[2],
						app_link_table[conn_id].bd_addr[1], app_link_table[conn_id].bd_addr[0]);
#else
      sprintf(cmd_buffer,"#condev#!%d![%02x%02x%02x%02x%02x%02x]<Y>\r\n",
						conn_id,
						app_link_table[conn_id].bd_addr[5], app_link_table[conn_id].bd_addr[4],
						app_link_table[conn_id].bd_addr[3], app_link_table[conn_id].bd_addr[2],
						app_link_table[conn_id].bd_addr[1], app_link_table[conn_id].bd_addr[0]);
    ...
    case GAP_AUTHEN_STATE_COMPLETE:
        {
            ...
            {
#if 0 //by vsop            
				data_uart_print("#sauth#!%d![%02x%02x%02x%02x%02x%02x]<Y>\r\n",
					conn_id,
					app_link_table[conn_id].bd_addr[5], app_link_table[conn_id].bd_addr[4],
					app_link_table[conn_id].bd_addr[3], app_link_table[conn_id].bd_addr[2],
					app_link_table[conn_id].bd_addr[1], app_link_table[conn_id].bd_addr[0]);
#else
			sprintf(cmd_buffer,"#sauth#!%d![%02x%02x%02x%02x%02x%02x]<Y>\r\n",
					conn_id,
					app_link_table[conn_id].bd_addr[5], app_link_table[conn_id].bd_addr[4],
					app_link_table[conn_id].bd_addr[3], app_link_table[conn_id].bd_addr[2],
					app_link_table[conn_id].bd_addr[1], app_link_table[conn_id].bd_addr[0]);
            ...
            {
#if 0 //by vsop            
               	data_uart_print("#sauth#!%d![%02x%02x%02x%02x%02x%02x]<N>\r\n",
						conn_id,
						app_link_table[conn_id].bd_addr[5], app_link_table[conn_id].bd_addr[4],
						app_link_table[conn_id].bd_addr[3], app_link_table[conn_id].bd_addr[2],
						app_link_table[conn_id].bd_addr[1], app_link_table[conn_id].bd_addr[0]);
#else
					sprintf(cmd_buffer,"#sauth#!%d![%02x%02x%02x%02x%02x%02x]<N>\r\n",
						conn_id,
						app_link_table[conn_id].bd_addr[5], app_link_table[conn_id].bd_addr[4],
						app_link_table[conn_id].bd_addr[3], app_link_table[conn_id].bd_addr[2],
						app_link_table[conn_id].bd_addr[1], app_link_table[conn_id].bd_addr[0]);
...
bool filter_scan_info_by_name(uint8_t *name, T_LE_SCAN_INFO *scan_info)
{
    uint8_t buffer[32];
    uint8_t pos = 0;
	            ...
	            case GAP_ADTYPE_LOCAL_NAME_COMPLETE:
	                {
						...
						if(strncmp((char *)buffer,(char *)name,5)==0)return true;
	                }
	                break;
	            default:
	                break;
...
void app_parse_scan_info(T_LE_SCAN_INFO *scan_info)
{
    ...
    while (pos < scan_info->data_len)
    {
        ...
        if ((length > 0x01) && ((pos + length) <= 31))
        {
            ...
            data_uart_print("app_parse_scan_info: AD Structure Info: AD type 0x%x, AD Data Length %d\r\n", type,
                             length - 1);
            ...
            case GAP_ADTYPE_FLAGS:
                {
                    ...
                    data_uart_print("GAP_ADTYPE_FLAGS: 0x%x\r\n", flags);
                }
                break;
            ...
            case GAP_ADTYPE_SERVICES_LIST_16BIT:
                {
                    ...
                    while (i >= 2)
                    {
                        data_uart_print("GAP_ADTYPE_16BIT_XXX: 0x%x\r\n", *p_uuid++);
                        i -= 2;
                    }
                }
                break;
            ...
            case GAP_ADTYPE_32BIT_COMPLETE:
                {
                    ...
                    while (i >= 4)
                    {
                        data_uart_print("GAP_ADTYPE_32BIT_XXX: 0x%x\r\n", *p_uuid++);
                        i -= 4;
                    }
                }
                break;
            ...
            case GAP_ADTYPE_SERVICES_LIST_128BIT:
                {
                    uint32_t *p_uuid = (uint32_t *)(buffer);
                    data_uart_print("GAP_ADTYPE_128BIT_XXX: 0x%8.8x%8.8x%8.8x%8.8x\r\n",
            ...
            case GAP_ADTYPE_POWER_LEVEL:
                {
                    data_uart_print("GAP_ADTYPE_POWER_LEVEL: 0x%x\r\n", scan_info->data[pos + 1]);
                }
                break;
            ...
            case GAP_ADTYPE_SERVICE_DATA:
                {
                    ...
                    data_uart_print("GAP_ADTYPE_SERVICE_DATA: UUID 0x%x, len %d, data %b\r\n", *p_uuid,
            ...
            case GAP_ADTYPE_APPEARANCE:
                {
                    ...
                    data_uart_print("GAP_ADTYPE_APPEARANCE: %d\r\n", *p_appearance);
                }
                break;
            ...
            case GAP_ADTYPE_MANUFACTURER_SPECIFIC:
                {
                    ...
                    data_uart_print("GAP_ADTYPE_MANUFACTURER_SPECIFIC: company_id 0x%x, len %d, data %b\r\n",
            ...
            default:
                {
                    ...
                    {
                        data_uart_print("  AD Data: Unhandled Data = 0x%x\r\n", scan_info->data[pos + i]);
                    }
                }
                break;
            }
            ...
            case GAP_ADTYPE_SERVICES_LIST_16BIT:
                {
                    ...
                    while (i >= 2)
                    {
						...
						//data_uart_print("  AD Data: UUID16 List Item %d = 0x%x\r\n", i / 2, *p_uuid);
                        if (*p_uuid == uuid)
                        {
                            return true;
                        }
...
T_APP_RESULT app_gap_callback(uint8_t cb_type, void *p_cb_data)
{
	...
	uint8_t devname[]="CMT4501-a11005e2";
    ...
    case GAP_MSG_LE_SCAN_INFO:
		...
		if (filter_scan_info_by_name(devname, p_data->p_le_scan_info))
        {
            //data_uart_print("Found simple ble service\r\n");
...
//by vsop for notify				
                APP_PRINT_INFO2("SIMP_V3_NOTIFY: value_size %d, value %b",
                                value_size, TRACE_BINARY(value_size, p_value));
				#if 0
				data_uart_print("#notify#!%d![]@%d{ ",conn_id,value_size);
				for(uint16_t i=0;i<value_size;i++)
				{
					sprintf(temp,"%02X",p_value[i]);
					data_uart_print("%s ",temp);					
				}
				data_uart_print("}\r\n");				
				#else
				if(value_size>FRAMING_PAYLOAD_MAX_LENGTH)value_size=FRAMING_PAYLOAD_MAX_LENGTH;
				sprintf(cmd_buffer,"#notify#!%d![]@%d{ ",conn_id,value_size);
				for(uint16_t i=0;i<value_size;i++)
				{
					sprintf(temp,"%02X ",p_value[i]);

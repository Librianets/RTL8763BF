...
void app_handle_io_msg(T_IO_MSG io_msg)
{
    ...
    case IO_MSG_TYPE_GPIO:
        current_val = RTC_GetCounter();
        //APP_PRINT_INFO1("RTC: %d",current_val);
        if (((current_val - previous_val) <= 102) && ((current_val - previous_val) >= 98))
        {
        ...
        {
            uint8_t time_difference = 100 - (current_val - previous_val);
            TIM_Cmd(TIM6, DISABLE);
            TIM_ChangePeriod(TIM6, time_difference * 40 * 1000 - 1);
            change_period = true;
        }
        TIM_Cmd(TIM6, ENABLE);
    ...
    case GAP_CONN_STATE_CONNECTED:
        {
            ...
            le_update_conn_param(conn_id, 80, 80, conn_latency,
                                 conn_supervision_timeout / 10, 160 * 2 - 2, 160 * 2 - 2);
            connect_id = conn_id;
            //os_timer_start(&xTimerADC);
        }
        break;

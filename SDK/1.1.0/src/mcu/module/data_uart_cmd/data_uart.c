...
bool FramingCheckSum(uint8_t* pu8Buffer, uint16_t u16Length, uint8_t* pu8CheckSum)
{
    uint8_t u8Sum = 0;

    if (pu8Buffer == NULL) {return false;} ///< Check if the pointers is null 
    if (pu8CheckSum == NULL) {return false;} ///< Check if the pointers is null 
...
void FramingWrapFrameUartPrint(uint8_t* pu8BufferIn, uint16_t u16BufferInLength)
{
    uint8_t u8CheckSum = 0;
	uint8_t u8BufferOut[512]={0};
	uint16_t BufferLength;
	uint16_t i;
  
    if (pu8BufferIn == NULL)           {return ;}     ///< Check if the pointer is null   
    if (u16BufferInLength > FRAMING_PAYLOAD_MAX_LENGTH)  {return ;}
    
    u8BufferOut[FRAMING_INDEX_SOH]         = FRAMING_SOH;
    u8BufferOut[FRAMING_INDEX_LENGTH_HIGH]      = (uint8_t)(u16BufferInLength>>8);
    u8BufferOut[FRAMING_INDEX_LENGTH_LOW]      =  (uint8_t)(u16BufferInLength&0xff);
    memcpy(&u8BufferOut[FRAMING_INDEX_PAYLOAD], pu8BufferIn, u16BufferInLength);
    
    if (FramingCheckSum(&u8BufferOut[FRAMING_INDEX_LENGTH_HIGH], u16BufferInLength + 2,&u8CheckSum) == false) {return ;} 
    
    u8BufferOut[FRAMING_INDEX_PAYLOAD +  u16BufferInLength ] = u8CheckSum;
    u8BufferOut[FRAMING_INDEX_PAYLOAD +  u16BufferInLength + 1] = FRAMING_EOT;
    
    BufferLength =  u16BufferInLength + 5;

	for(i=0;i<BufferLength;i++)
	{
		data_uart_send_char(u8BufferOut[i]);
	}

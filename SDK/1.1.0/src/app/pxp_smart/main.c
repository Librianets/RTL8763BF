...
#define DEFAULT_SCAN_WINDOW       0x18
...
static const uint8_t adv_data[] =
{
    ...
    0x02,//LO_WORD(GATT_UUID_IMMEDIATE_ALERT_SERVICE),
    0x18,//HI_WORD(GATT_UUID_IMMEDIATE_ALERT_SERVICE),
...
void app_le_gap_init(void)
{
    ...
    uint8_t  device_name[GAP_DEVICE_NAME_LEN] = "PXP_SMART";
...
{
    uint8_t tmpVal, keySva, keyMva;

    APP_PRINT_INFO0("System_Handler");
    NVIC_DisableIRQ(System_IRQn);
    keySva = GPIO_ReadInputDataBit(GPIO_GetPin(KEY_S));
    if (keySstatus != keySva)
    {
        KEY_S_INT_Handle();
    }
    keyMva = GPIO_ReadInputDataBit(GPIO_GetPin(KEY_M));
    if (keyMstatus != keyMva)
    {
        KEY_M_INT_Handle();
    }
...
{
    Pad_Config(KEY_S, PAD_SW_MODE, PAD_IS_PWRON, PAD_PULL_UP, PAD_OUT_DISABLE, PAD_OUT_LOW);
    Pad_Config(LED_S, PAD_SW_MODE, PAD_IS_PWRON, PAD_PULL_NONE, PAD_OUT_ENABLE, PAD_OUT_LOW);
    Pad_Config(KEY_M, PAD_SW_MODE, PAD_IS_PWRON, PAD_PULL_UP, PAD_OUT_DISABLE, PAD_OUT_LOW);
    Pad_Config(LED_M, PAD_SW_MODE, PAD_IS_PWRON, PAD_PULL_NONE, PAD_OUT_ENABLE, PAD_OUT_LOW);
    Pad_Config(BEEP, PAD_SW_MODE, PAD_IS_PWRON, PAD_PULL_NONE, PAD_OUT_ENABLE, PAD_OUT_LOW);
    System_WakeUpDebounceTime(0x8);
    if (keySstatus)
    {
        System_WakeUpPinEnable(KEY_S, PAD_WAKEUP_POL_LOW, PAD_WK_DEBOUNCE_ENABLE);
    }
    else
    {
        System_WakeUpPinEnable(KEY_S, PAD_WAKEUP_POL_HIGH, PAD_WK_DEBOUNCE_ENABLE);
    }

    if (keyMstatus)
    {
        System_WakeUpPinEnable(KEY_M, PAD_WAKEUP_POL_LOW, PAD_WK_DEBOUNCE_ENABLE);
    }
    else
    {
        System_WakeUpPinEnable(KEY_M, PAD_WAKEUP_POL_HIGH, PAD_WK_DEBOUNCE_ENABLE);
    }
}
/**

...
bool simp_ble_client_read_by_handle(uint8_t conn_id, T_SIMP_READ_TYPE read_type)
{
    ...
    default:
		APP_PRINT_INFO1("vsop----read_by_handle------return=%d",read_type); 		
        return false;
    }
	APP_PRINT_INFO1("vsop----read_by_handle------read_type=%d",read_type); 

    if (hdl_valid)
    {
    	APP_PRINT_INFO1("vsop----read_by_handle------hdl_valid=%d",hdl_valid); 
        if (client_attr_read(conn_id, simp_client, handle) == GAP_CAUSE_SUCCESS)
        {
           	APP_PRINT_INFO1("vsop----read_by_handle------okkkkk=%d",hdl_valid); 
            return true;
        }
    }
...
bool simp_ble_client_read_by_uuid(uint8_t conn_id, T_SIMP_READ_TYPE read_type)
{
	...
	APP_PRINT_INFO1("vsop----read_by_uuid------read_type=%d",read_type); 
    ...
    case SIMP_READ_V3_NOTIFY_CCCD:
        ...
        uuid16 = GATT_UUID_CHAR_CLIENT_CONFIG;
   		APP_PRINT_INFO0("vsop----read_by_uuid------ SIMP_READ_V3_NOTIFY_CCCD");		 
        break;
    case SIMP_READ_V4_INDICATE_CCCD:
...
bool simp_ble_client_set_v3_notify(uint8_t conn_id, bool notify)
{
	...
	APP_PRINT_INFO0("vsop----simp_ble_client_set_v3_notify enter");	
    ...
    if (simp_table[conn_id].hdl_cache[HDL_SIMBLE_V3_NOTIFY_CCCD])
    {
        ...
        {
        	APP_PRINT_INFO0("vsop----simp_ble_client_set_v3_notify enter---write fail");	
            return true;
        }
    }
	...
	APP_PRINT_INFO1("vsop----simp_ble_client_set_v3_notify =	0x%02X",simp_table[conn_id].hdl_cache[HDL_SIMBLE_V4_INDICATE_CCCD]);	
...
bool simp_ble_client_set_v4_ind(uint8_t conn_id, bool ind)
{
	...
	APP_PRINT_INFO1("vsop----HDL_SIMBLE_V4_INDICATE_CCCD =  0x%02X",simp_table[conn_id].hdl_cache[HDL_SIMBLE_V4_INDICATE_CCCD]);	
...
{
    APP_PRINT_INFO2("simp_ble_client_discover_result_cb: result_type %d , 0x%02X", result_type,result_data.p_char_uuid16_disc_data->value_handle);
    if (simp_table[conn_id].disc_state == DISC_SIMP_START)
    {
        uint16_t handle;
        uint16_t *hdl_cache;
        hdl_cache = simp_table[conn_id].hdl_cache;
		APP_PRINT_INFO2("vsop simp_ble_client_discover_result_cb: result_type %d , 0x%02X", result_type,result_data.p_char_uuid16_disc_data->value_handle);
        ...
        case DISC_RESULT_CHAR_UUID16:
			...
			APP_PRINT_INFO1("vsop result_data DISC_RESULT_CHAR_UUID16: 0x%02X", result_data.p_char_uuid16_disc_data->uuid16);
            ...
            case GATT_UUID_CHAR_SIMPLE_V3_NOTIFY:
				APP_PRINT_INFO1("vsop+++++++++++++GATT_UUID_CHAR_SIMPLE_V3_NOTIFY, handle=%d",handle);
                hdl_cache[HDL_SIMBLE_V3_NOTIFY] = handle;
                break;

            case GATT_UUID_CHAR_SIMPLE_V4_INDICATE:
				APP_PRINT_INFO1("vsop+++++++++++++GATT_UUID_CHAR_SIMPLE_V4_INDICATE, handle=%d",handle);
        ...
        case DISC_RESULT_CHAR_DESC_UUID16:
			...
			APP_PRINT_INFO1("vsop 1 DISC_RESULT_CHAR_DESC_UUID16: uuid16 , 0x%02X", result_data.p_char_desc_uuid16_disc_data->uuid16);
            if (result_data.p_char_desc_uuid16_disc_data->uuid16 == GATT_UUID_CHAR_CLIENT_CONFIG)
            {
				...
				APP_PRINT_INFO1("vsop 2 DISC_RESULT_CHAR_DESC_UUID16: handle , 0x%02X",handle);
				
                if ((handle > hdl_cache[HDL_SIMBLE_V3_NOTIFY])
                    && (handle < hdl_cache[HDL_SIMBLE_V4_INDICATE]))
                {
                	APP_PRINT_INFO1("vsopxxxxxxxxxxxxxxxxx1, handle=%d",handle);
                         ...
                         (hdl_cache[HDL_SIMBLE_V4_INDICATE_CCCD] == 0))
                {
					...
					APP_PRINT_INFO1("vsopxxxxxxxxxxxxxxxxx2, handle=%d",handle);
                                           ...
                                           uint16_t handle, uint16_t value_size, uint8_t *p_value)
{
    ...
    else if (handle == hdl_cache[HDL_SIMBLE_V4_INDICATE_CCCD])
    {
        cb_data.cb_content.read_result.type = SIMP_READ_V4_INDICATE_CCCD;
			APP_PRINT_INFO0("vsop------- indicate _cccd 1");		
        if (cause == GAP_SUCCESS)
        {
            uint16_t ccc_bit;
            if (value_size != 2)
            ...
            LE_ARRAY_TO_UINT16(ccc_bit, p_value);
			APP_PRINT_INFO1("vsop------- indicate _cccd =  %d",ccc_bit);
                                                        ...
                                                        uint16_t value_size, uint8_t *p_value)
{
    ...
    if (handle == hdl_cache[HDL_SIMBLE_V3_NOTIFY])
    {
    	APP_PRINT_INFO1("vsop------------HDL_SIMBLE_V3_NOTIFY %d",handle);
    ...
    {
    	APP_PRINT_INFO1("vsop------------HDL_SIMBLE_V4_INDICATE %d",handle);
    ...
    {
    	APP_PRINT_INFO1("vsop------------no handle %d",handle);
        return app_result;
    }
    /* Inform application the notif/ind result. */
    if (simp_client_cb)
    {
    	APP_PRINT_INFO1("vsop------------simp_client_cb %d",handle);
        app_result = (*simp_client_cb)(simp_client, conn_id, &cb_data);
    }
	APP_PRINT_INFO1("vsop------------return %d",handle);

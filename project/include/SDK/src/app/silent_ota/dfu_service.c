...
DATA_RAM_FUNCTION bool flash_wait_busy(void)
{
...
DATA_RAM_FUNCTION bool app_flash_cmd_tx(uint8_t cmd, uint8_t data_len, uint8_t *data_buf)
{
    ...
    if (retval == true && !flash_wait_busy())
    {
        retval = false;
    }
    //restore ctrl0 and addr_len register
...
DATA_RAM_FUNCTION bool app_flash_cmd_tx(uint8_t cmd, uint8_t data_len, uint8_t *data_buf)
{
    ...
    DFU_PRINT_INFO1("flash_wait_busy ..%x", retval);
...
bool sil_dfu_copy_img(uint16_t image_id, uint32_t dlAddress, uint32_t dlSize)
{
    ...
    if ((image_id != AppData1) && (image_id != AppData2))
    {
        error_code = __LINE__;
        goto L_Return;
    }

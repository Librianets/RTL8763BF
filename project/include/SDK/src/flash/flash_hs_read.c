...
DATA_RAM_FUNCTION void FLASH_GDMA_HANDLER(void)
{
...
*/
bool flash_split_read_locked(uint32_t start_addr, uint32_t data_len, uint8_t *data,
                             uint32_t *counter)
{
    if (!flash_lock(FLASH_LOCK_USER_MODE_READ))
    {
        return false;
    }

    *counter = flash_split_read(start_addr, data_len, data);
